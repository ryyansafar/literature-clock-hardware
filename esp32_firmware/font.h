/**
 * font.h — 5x7 Bitmap Font for LED Matrix
 *
 * Each character is 5 pixels wide, 7 pixels tall.
 * Stored as 7 bytes per character, each byte's lower 5 bits = one row.
 * Bit 4 (0x10) = leftmost pixel, Bit 0 (0x01) = rightmost pixel.
 *
 * ASCII 32 (space) through ASCII 126 (~), 95 characters total.
 * Index = ascii_code - 32.
 */
...
// 5x7 font data: 7 rows per character, 95 characters
const uint8_t PROGMEM font5x7[FONT_CHAR_COUNT][7] = {
    // 32: ' '  (space)
...
/**
 * Get the bitmap row data for a character.
 * Returns pointer to 7 bytes in PROGMEM, or NULL if out of range.
 */
inline const uint8_t* getCharBitmap(char c) {
    if (c < FONT_FIRST_CHAR || c > FONT_LAST_CHAR) return nullptr;
